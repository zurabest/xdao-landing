...
const int MAX_LOGOS = 1;
uniform vec4  uLogoRects[MAX_LOGOS];     // x,y = bottomLeft UV, z,w = width/height
uniform float uLogoStrengths[MAX_LOGOS]; // current brightness per slot
...
void main(){
  ...
  // --- Logo silhouette particle system ---
  {
    vec2 screenUV = gl_FragCoord.xy / uResolution;
    for (int i = 0; i < MAX_LOGOS; i++) {
      float str = uLogoStrengths[i];
      if (str <= 0.0) continue;
      vec4 rect = uLogoRects[i];
      vec2 logoUV = (screenUV - rect.xy) / rect.zw;
      if (logoUV.x >= 0.0 && logoUV.x <= 1.0 && logoUV.y >= 0.0 && logoUV.y <= 1.0) {
        float alpha = texture(uLogoTex, vec2(1.0 - logoUV.x, 1.0 - logoUV.y)).a;
        feed += alpha * str;
      }
    }
  }
...
interface PixelBlastProps {
    logoSrc?: string;           // path to logo image (default: '/xdao-logo.png')
    logoStrength?: number;      // max brightness boost (0 = off, default 0.9)
    logoCycleSeconds?: number;  // full fade-in+out period in seconds (default 8)
...
const PixelBlast = ({
    ...
    useEffect(() => {
        ...
        if (mustReinit) {
            ...
            placeholderTex.needsUpdate = false;
            const loaderImg = new Image();
            loaderImg.crossOrigin = 'anonymous';
            let logoNatW = 1;
            let logoNatH = 1;
            loaderImg.onload = () => {
                const logoTex = new THREE.Texture(loaderImg);
                logoTex.minFilter = THREE.LinearFilter;
                logoTex.magFilter = THREE.LinearFilter;
                logoTex.needsUpdate = true;
                logoNatW = loaderImg.naturalWidth || loaderImg.width;
                logoNatH = loaderImg.naturalHeight || loaderImg.height;
                if (threeRef.current) {
                    threeRef.current.uniforms.uLogoTex.value = logoTex;
                    threeRef.current.logoTex = logoTex;
                }
            };
            loaderImg.src = logoSrc;
            ...
            const uniforms = {
                ...
                uLogoTex: { value: placeholderTex },
                uLogoRects: { value: Array.from({ length: 1 }, () => new THREE.Vector4(0, 0, 0, 0)) },
                uLogoStrengths: { value: new Float32Array(1) },
            };
            ...
            const animate = () => {
                ...
                for (let i = 0; i < LOGO_SLOTS; i++) {
                    ...
                    uniforms.uLogoStrengths.value[i] = Math.max(0, str);
                    uniforms.uLogoRects.value[i].set(p.x, p.y, p.w, p.h);
                }
